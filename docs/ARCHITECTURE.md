...
- **Ops** — `POST /v1/ops/recover` parks stale processing claims; `POST /v1/ops/sweep` has the boss ping the hive about review/failed/reconsider. `MAILROOM_JUDGE_VERIFY=off` skips the judge band. Reconsideration flags archived filings that are hollow, conflicted, or low-confidence.
...
**LLM failures** park the document on human review instead of crashing the watcher loop. JSON replies are parsed through a fence-tolerant decoder.
